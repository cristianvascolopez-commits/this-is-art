...
async function sendLeadWelcome(email) {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_APP_PASSWORD) {
    console.error('[Lead Email] Credenciales de email no configuradas');
    return;
  }

  if (!email || typeof email !== 'string') {
    console.error('[Lead Email] Email inválido:', email);
    return;
  }

  const transporter = getTransporter();

  // Email para la barbería (notificación de nuevo lead)
  const htmlBarberia = `<div style="font-family:Arial,sans-serif;max-width:480px;background:#0a0a0a;color:#f0f0f0;border-radius:12px;overflow:hidden;">
  ...
  // Email para el cliente (descuento y bienvenida)
  const htmlCliente = `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#0a0a0a;color:#f0f0f0;border-radius:16px;overflow:hidden;">
    <div style="background:#111;padding:36px 40px;text-align:center;border-bottom:1px solid #1e1e1e;">
      <div style="font-size:2rem;margin-bottom:8px;">✂</div>
  ...
  results.forEach((r, i) => {
    if (r.status === 'fulfilled') {
      console.log(`[Lead Email] Enviado OK (${i === 0 ? 'cliente' : 'barbería'}):`, r.value.messageId);
    } else {
      console.error(`[Lead Email] Error (${i === 0 ? 'cliente' : 'barbería'}):`, r.reason?.message);
    }
  });
}
