...
const PREFIX = '🤖 AURABOT';

async function createAppointment({ nombre, servicio, fecha, hora, telefono, empresa }) {
  ...
  const event = {
    summary: `${PREFIX} — ${nombre} · ${servicio}`,
    description: `Cliente: ${nombre}\nEmpresa: ${empresa || 'no indicada'}\nServicio: ${servicio}\nTeléfono: ${telefono || 'no indicado'}\nReservado via chat web aurabotbcn.es`,
...
async function getAvailableSlots(dateStr) {
  ...
  const timeMin = new Date(`${dateStr}T09:00:00+02:00`).toISOString();
...
async function searchAppointments(query) {
  ...
  return (res.data.items || [])
    .filter(e => e.summary && e.summary.includes('AURABOT'))
    .map(e => ({
      id: e.id,
      summary: e.summary,
